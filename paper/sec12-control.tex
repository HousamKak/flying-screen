...
\begin{figure}[htbp]
\centering
\input{figures/sec12_bandwidth}
\caption{The frequency ladder of the design's cascade on a logarithmic axis,
with the gains of \cref{mod:gains}. Each loop is at least a factor of three
slower than the one it relies on: position at $\sqrt{K_p}$, attitude at
$\sqrt{K_R}$, the motors at $1/\tau_m$. The governor's filter at $\sqrt{k_p}$
shapes the reference and closes no loop on the vehicle. The first bending mode
of the arms and the sampling rate of the controller lie one and two orders of
magnitude above the fastest loop, which is the condition under which the
airframe may be treated as rigid and the sampled controller as continuous.}
\label{fig:bandwidth}
\end{figure}
...
\begin{figure}[tbp]
\centering
\input{figures/sec12_attainable}
...
\cref{fig:bandwidth} places these values on one frequency axis.
...
\begin{figure}[tbp]
\centering
\input{figures/sec12_turnaround}
