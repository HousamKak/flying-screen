...
destroys the structure of the problem. \Cref{fig:budget} shows the budget of
the design of \cref{sec:results} with the dependence of each group on $m$ that
the rest of this section derives.

\begin{figure}[tbp]
\centering
\input{figures/sec05_budget}
\caption{The mass budget \cref{eq:budget} of the design of \cref{sec:results},
to scale, from the component models of \cref{sec:components}; masses in grams.
The four groups enter the closure differently. The fixed load does not depend
on $m$; structure and propulsion grow in proportion to it
(\cref{mod:fs,mod:sm}); the battery grows with the hover power, as $m^{3/2}$
at fixed rotor area (\cref{mod:battery,cor:hover}). Only the last is
nonlinear, and it is the one that makes \cref{eq:closure} fold.}
\label{fig:budget}
\end{figure}
...
\Cref{fig:closuregraph} reads \cref{eq:closure} as supply against demand.

\begin{figure}[tbp]
\centering
\input{figures/sec05_closure}
\caption{\Cref{eq:closure} as a balance between the mass left after structure
and propulsion, $\alpha m$, and the mass the fixed load and the battery demand,
$\Mzero + \beta m^{3/2}$, with the coefficients of the worked example. A
design exists where the two meet. For $\Mzero = 0.40$ they meet twice, at a
light root $m_-$ and a heavy root $m_+$, with a surplus between them. At
$\Mzero = \Mmax = 0.741$ the demand curve is tangent to the line at
$m^* = 3.49$. Above it, as for $\Mzero = 1.00$ and for the worked example's
$\Mzero = 0.762$, they never meet and no mass closes the budget.
\Cref{sec:fold} proves this trichotomy.}
\label{fig:closuregraph}
\end{figure}
...
\end{itemize}
\Cref{fig:plaw} shows the family.

\begin{figure}[tbp]
\centering
\input{figures/sec05_plaw}
\caption{The generalised closure \cref{eq:closure-q} for five disk-area
exponents, all anchored to the same rotor at $m_0 = \SI{2}{\kilo\gram}$ so
that the demand curves $\Mzero + \beta_q m^q$ pass through one point, with the
worked-example $\alpha$, $\beta$ and $\Mzero = \SI{0.5}{\kilo\gram}$. For
$q > 1$ the demand is convex and eventually overtakes the supply line, so a
second root and a fold exist; the smaller $q - 1$, the farther out they lie,
and for $q = \tfrac54$ they are beyond the plotted range. At $q = 1$ (constant
disk loading, \cref{cor:constdl}) the demand is a line, and a root exists
exactly when its slope is below $\alpha$. For $q < 1$ the demand is concave and
is overtaken by the supply for good.}
\label{fig:plaw}
\end{figure}
