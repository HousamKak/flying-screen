...
\cref{eq:exchange} (\cref{fig:recirc}) the machine passes a room's volume of air through its disks
...
anchor, whose calibration spread alone is \SI{1.5}{\decibel}. No propagation
...
\Cref{fig:hexfail} shows both failures.

\begin{figure}[tbp]
\centering
\input{figures/sec18_hexfail}
\caption{Rotor failure in the engine's layouts, top view, arcs showing each
rotor's spin. Left: a quadrotor that loses a rotor has three inputs for four
wrench components; with the remaining spins unbalanced it cannot hover at
zero yaw rate, which is why the relaxed-hover controllers of~\cite{mueller2014}
let it spin. Right: a hexacopter that loses the opposite pair 1 and 4 keeps
four rotors, but their allocation columns satisfy
$\mat M_{:,2}+\mat M_{:,5}=\mat M_{:,3}+\mat M_{:,6}$, so $\rank\mat M=3$ and
every moment the four can produce satisfies the relation printed below it:
roll, pitch and yaw can no longer be commanded independently.}
\label{fig:hexfail}
\end{figure}
...
\begin{figure}[tbp]
\centering
\input{figures/sec18_latency}
\caption{Why estimator latency erodes the keep-out guarantee, not to scale.
The governor holds the reference outside the sphere of radius $\varrho$ about
the estimated head $\hat{\vect c}=\vect c(t-\tau)$, and the vehicle stays
within $\varepsilon$ of the reference. If the head has moved
$\|\dt{\vect c}\|\tau$ since it was seen, the sphere is displaced by that much,
and in the direction of motion the vehicle can be closer to the true head
than \cref{prop:governor}(iv) allows. At \SI{1.4}{\metre\per\second} and
\SI{100}{\milli\second} the displacement is \SI{0.14}{\metre}, larger than the
whole margin $\varepsilon=\SI{0.10}{\metre}$.}
\label{fig:latency}
\end{figure}

\Cref{fig:latency} draws the effect.
...
The integration is converged at the design step (\cref{tab:numerics}). The
controller rate is a different matter: it defines the sampled-data loop, and
doubling it moves the peak tracking error by \SI{\rCtlDTrack}{\milli\metre}, so
the rate simulated must be the rate the real autopilot runs at. The
differential evolution of \cref{eq:outerprog} is
