...
\cref{sec:results} at $t_f=\SI{4280.0502}{\second}$ has a positive island
about \SI{1}{\percent} wide in mass, with roots at \SI{4.5678}{\kilo\gram}
and \SI{4.6173}{\kilo\gram}, against a grid spacing of \SI{35}{\percent}.
The grid alone straddled it and reported that no mass closes;
\cref{fig:rootsearch} shows the case.

\begin{figure}[tbp]
\centering
\input{figures/sec15_rootsearch}
\caption{The component residual $\mathcal{F}$ of \cref{eq:fpclosure} for the
design of \cref{sec:results} at $t_f=\SI{4280.05}{\second}$, just short of
the fold. (a) On the scale of the whole search $\mathcal{F}$ appears merely to
touch zero; the geometric grid of ratio $1.35$ samples it at the dots, and the
samples on either side of the positive stretch, at \SI{\fdGridLo}{\kilo\gram}
and \SI{\fdGridHi}{\kilo\gram}, are both negative. (b) Magnified, the positive
island is \SI{\fdIslandPct}{\percent} wide in mass and \SI{\fdPeakF}{\gram}
high, bounded by the roots $m_\pm$. The engine finds it by refining every
sampled local maximum with a bounded maximisation in $\ln m$; the grid alone
reported that no mass closes.}
\label{fig:rootsearch}
\end{figure}
...
eight or more for plain iteration. \Cref{fig:convergence} compares the two on
one mission.

\begin{figure}[tbp]
\centering
\input{figures/sec15_convergence}
\caption{Convergence of the battery loop on the pacing mission, flown with an
\SI{\fdConvWindow}{\second} window so that plain iteration can be run to eight
updates at reasonable cost. Plain iteration $\mu_{k+1}=\mathcal{H}(\mu_k)$
contracts linearly, the residual falling by a factor of about
\fdPlainRate{} per update, which is $\mathcal{H}'$ of \cref{prop:contraction}
at this design. The guarded secant step \cref{eq:secant} takes the same first
step and is below the tolerance on the third update. Each point costs one full
simulation.}
...
\begin{figure}[tbp]
\centering
\input{figures/sec15_rk4}
\caption{(a) The stability region of classical RK4,
$\{z:|1+z+z^2/2+z^3/6+z^4/24|\le1\}$, which reaches $-2.79$ on the negative
real axis. (b) The products $h\lambda$ for the poles of the design's loops at
$h=\SI{4}{\milli\second}$: the motor lag, the attitude loop
$s^2+K_\omega s+K_R$, the position loop $s^2+K_ds+K_p$ and the governor's
filter. The fastest, the motor, sits at $h\lambda=\fdHlMotorRe$, about thirty
times inside the boundary, so the step is set by accuracy and by the sampling
of the controller, not by stability.}
\label{fig:rk4}
\end{figure}
...
error. \Cref{fig:sampled} shows the structure of one step.

\Cref{tab:numerics} separates the two. On the turnaround, the hardest
mission, the controller is held at \SI{250}{\hertz} while the plant is
integrated with one to eight RK4 substeps per control period under the same
held commands. The mean power changes by a relative \num{\rSubDP}, and the
peak tracking error and the minimum rotor clearance by less than
\SI{\rSubDTrack}{\milli\metre} and \SI{\rSubDClear}{\milli\metre}: at the
design step the integration is converged, and the figures of
\cref{sec:results} are properties of the sampled-data loop, not of the
integrator. Halving the controller period instead changes the loop being
simulated, not the accuracy of simulating it. It moves the mean power by
\SI{\rCtlDP}{\percent} and the rotor clearance by
\SI{\rCtlDClear}{\milli\metre}, but raises the peak tracking error from
\SI{\rCtlTrackBase}{\milli\metre} to \SI{\rCtlTrackHalf}{\milli\metre}, still
inside the margin $\varepsilon$. The peak tracking error is therefore the one
reported quantity that is sensitive to the controller rate, and its distance
to $\varepsilon$ is the number to recheck for an autopilot running at a
different rate.

\begin{table}[tbp]
\centering
\caption{Integration convergence on the turnaround at a fixed controller rate
of \SI{250}{\hertz}: the plant is integrated with one to eight RK4 substeps
per control period under the same held commands. Every reported quantity is
unchanged to the digits shown.}
\label{tab:numerics}
\small
\begin{tabular}{@{}rrrrrr@{}}
\toprule
Substeps & RK4 step & Mean power & Peak $e_{\mathrm{track}}$ & $e_{\mathrm{lag}}$ rms & Rotor clearance\\
\midrule
\inputrows{results/tab_numerics_rows}
\end{tabular}
\end{table}

\begin{figure}[tbp]
\centering
\input{figures/sec15_sampled}
\caption{The sampled-data structure of every simulation. At $t_k$ the
controller reads the state and the references at $t_k$, and the telemetry
records both at that instant; the controller's output $u_k$ is then held over
the step while RK4 evaluates the plant four times, at $t_k$, twice at
$t_k+h/2$ and at $t_{k+1}$. The integration error is $O(h^4)$ for the plant
under the held input. The difference between this sampled loop and a
continuous-feedback idealisation belongs to the controller, which is sampled
at \SI{250}{\hertz} as a digital autopilot would be.}
\label{fig:sampled}
\end{figure}
