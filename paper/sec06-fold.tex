...
\Cref{fig:trichotomy} draws the three cases for the worked example of
\cref{sec:closure}.

\begin{figure}[tbp]
\centering
\input{figures/sec06_trichotomy}
\caption{The closure function $F(m)=\alpha m-\beta m^{3/2}$ of \cref{eq:F} for
the worked example of \cref{sec:closure}, $\alpha=0.6366$ and
$\beta=\SI{0.2271}{\kilo\gram^{-1/2}}$. $F$ is strictly concave with its
maximum $\Mmax=\SI{0.741}{\kilo\gram}$ at $m^*=\SI{3.49}{\kilo\gram}$
(\cref{lem:concave}). A horizontal line at height $\Mzero$ meets it twice, once
or never, which is \cref{thm:trichotomy}; at $\Mzero=\SI{0.45}{\kilo\gram}$ the
roots are $m_-=\SI{1.14}{\kilo\gram}$ and $m_+=\SI{6.16}{\kilo\gram}$. On the
shaded interval $F(m)>\Mzero$: a machine of that mass could carry more than it
is asked to, and by \cref{thm:iteration} the sizing iteration moves down there.}
...
The bifurcation diagram is \cref{fig:saddlenode}.

\begin{figure}[tbp]
\centering
\input{figures/sec06_saddlenode}
\caption{The roots of $F(m)=\Mzero$ against the load for the worked example,
drawn as the parametric curve $(F(m),m)$. The light branch $m_-$ (solid) and
the heavy branch $m_+$ (dashed) meet at the fold $(\Mmax,m^*)$ and annihilate;
beyond it no mass closes. Near the fold both branches follow the square-root
law \cref{eq:sqrtlaw} (dotted), with $K=q(q-1)\beta(m^*)^{q-2}$, so
$\dd m_-/\dd\Mzero\to\infty$ as $\Mzero\uparrow\Mmax$: the design mass becomes
infinitely sensitive to the load just before it ceases to exist.}
...
\Cref{fig:cobweb} shows the iteration as a cobweb diagram.

\begin{figure}[tbp]
\centering
\input{figures/sec06_cobweb}
\caption{Cobweb diagram of the sizing iteration \cref{eq:iteration} for the
worked example at $\Mzero=\SI{0.45}{\kilo\gram}$. Each vertical step evaluates
$\Phi$ and each horizontal step returns to the diagonal. (a) Started below
$m_-$ or between the roots, the iterates go monotonically to $m_-$; started
above $m_+$ they escape, as \cref{thm:iteration}(iii) and (iv) state. The
fixed points are where $\Phi$ crosses the diagonal, $\Phi$ being below it
exactly on $(m_-,m_+)$. (b) Near $m_-$ the slope of $\Phi$ is
$1-F'(m_-)=0.73<1$, so the error shrinks by that factor at each step.}
\label{fig:cobweb}
\end{figure}
...
classifies slow convergence separately from divergence. \Cref{fig:slowdown}
quantifies the effect.
\end{remark}

\begin{figure}[tbp]
\centering
\input{figures/sec06_slowdown}
\caption{Critical slowing down of the sizing iteration, for the worked example.
(a) The contraction factor $\Phi'(m_-)=1-F'(m_-)$ of \cref{thm:iteration}(ii)
tends to one as the utilisation $\Mzero/\Mmax$ tends to one. (b) The number of
plain iterations from $m_0=0$ to a relative error of $10^{-6}$ grows like
$(1-\Mzero/\Mmax)^{-1/2}$, because by \cref{eq:sqrtlaw}
$F'(m_-)=K(m^*-m_-)\approx\sqrt{2K\Mmax(1-\Mzero/\Mmax)}$. A fixed iteration
budget therefore cannot separate a design close to the fold from one beyond
it.}
\label{fig:slowdown}
\end{figure}
...
\Cref{fig:qshapes} contrasts the three regimes.

\begin{figure}[tbp]
\centering
\input{figures/sec06_qshapes}
\caption{The shape of $F(m)=\alpha m-\beta m^q$ decides whether a fold exists
(\cref{thm:pfold}), drawn with $\alpha=1$ and illustrative $\beta$. For $q>1$
($p<1$) $F$ is concave with an interior maximum, so a load meets it twice or
not at all. For $q=1$ it is a straight line, met once if $\alpha>\beta$. For
$q<1$ it dips below zero to a minimum at $m_{\min}=(q\beta/\alpha)^{1/(1-q)}$
and then grows without bound, so every positive load is met exactly once.}
\label{fig:qshapes}
\end{figure}
